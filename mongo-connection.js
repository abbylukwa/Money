...
async function groupMetadata(jid, dbName) {
  const database = await getDb(dbName);
...
async function saveGroupMetadata(jid, metadata, dbName) {
  const database = await getDb(dbName);
  const groupMetadataCollection = database.collection('group_metadata');
  try {
...
async function saveMessages(upsert, dbName) {
  const database = await getDb(dbName);
...
async function loadMessage(id, jid = null, dbName) {
  const database = await getDb(dbName);
  const messagesCollection = database.collection('messages');
...
async function saveContact(contact, dbName) {
...
async function saveReceipts(updates, dbName) {
  const database = await getDb(dbName);
